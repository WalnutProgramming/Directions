...
'use strict';
import { hallways, LEFT, RIGHT, FRONT, BACK, stairConnections, hallwayConnections, Room, Turn, Stairs, Fork } from './hallwayDefinition.js';
...
function getHallwayIndexAndIndexFromNode(nodeId) {
  const inds = hallways.map(h => h.partList.findIndex(r => 'nodeId' in r && r.nodeId == nodeId));
  const hallwayInd = inds.findIndex(a => a != -1);
...
function isConnectionStairs(id1, id2) {
  return stairConnections.findIndex(arr => arr.includes(id1) && arr.includes(id2)) != -1;
}

function getStairConnectionInstruction(id1, id2, numFlights) {
  const goingUp = stairConnections.find(arr => arr.includes(id1) && arr.includes(id2) && arr.indexOf(id2) > arr.indexOf(id1));
  const maybeS = numFlights > 1 ? 's' : '';
  return `Go ${goingUp ? 'up' : 'down'} ${numFlights} floor${maybeS} of stairs\n`;
}
...
function getDirections(from, to) {
  ...
  const closestNodeFromInd = hallways[fromHallwayInd].idOfClosestNodeToIndex(fromInd);
  ...
  for (let i = 1; i < shortest.length; i++) {
    ...
    const [prevHallwayInd, prevInd] = getHallwayIndexAndIndexFromNode(shortest[i - 1]);
    if (isConnectionStairs(shortest[i-1], shortest[i]) /* going up or down stairs */) {
      directions += hallways[currentHallwayInd].getDirectionsFromIndices(currentInd, prevInd);
      const numStairFlights = Math.ceil(graph[shortest[i-1]][shortest[i]]);
      directions += getStairConnectionInstruction(shortest[i-1], shortest[i], numStairFlights);
      [currentHallwayInd, currentInd] = getHallwayIndexAndIndexFromNode(shortest[i]);
  ...
  directions += hallways[currentHallwayInd].getDirectionsFromIndices(currentInd, toInd);
  return directions;
}
